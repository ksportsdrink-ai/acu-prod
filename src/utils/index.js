...
export function parseQuickInput(raw, baseHour) {
  ...
  if (!nameParts.length)
    return { error: '환자명을 입력하세요' }
  if (isNaN(count) || count < 1)
    return { error: `침 갯수 오류: "${countStr}"` }
  ...
  return {
    ...
    room,
    floor:            detectFloor(room),
    patientAnonymized: anonymize(nameParts.join(' ')),
    needleCount:      count,
    preview: {
      time:    `${pad(hour)}:${pad(minute)}`,
      floor:   detectFloor(room),
      patient: anonymize(nameParts.join(' ')),
      count,
    }
  }
}
...
export function exportCSV(tasks, dateStr) {
  const headers = [
    '날짜','예약시간','호실','층','환자(익명)','침갯수','진료과',
    '등록자','진행담당자','진행시작','완료담당자','완료시각','상태','지연사유','메모'
  ]
